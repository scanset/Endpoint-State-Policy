...
impl RecordAccessMut for JsonRecord {
    ...
    fn remove_field(&mut self, path: &FieldPath) -> Result<Option<ResolvedValue>, RecordError> {
        ...
        let all_but_last = path.components.len().saturating_sub(1);
        for component in path.components.get(..all_but_last).unwrap_or(&[]) {
        ...
        // Safe because we checked is_empty() at the start of the function
        let final_component = match path.components.last() {
            Some(c) => c,
            None => return Ok(None), // Defensive - should never happen
        };
        match current {
            serde_json::Value::Object(map) => {
                // Convert PathComponent to String for map key
