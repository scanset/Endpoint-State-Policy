...
// execute_intersection - lines 327, 332, 336
fn execute_intersection(
    ...
    // FIXED: Use .first() instead of indexing
    let Some(first_list) = object_lists.first() else {
        return Ok(Vec::new());
    };

    if object_lists.len() == 1 {
        return Ok(first_list.clone());
    }
    ...
    for object in first_list {
        ...
        // FIXED: Use .get() instead of slicing
        let exists_in_all = object_lists
            .get(1..)
            .unwrap_or(&[])
            .iter()
            .all(|other_list| {
...
// execute_complement - lines 365, 366
fn execute_complement(
    ...
    // FIXED: Use .first() and .get(1) instead of indexing
    let Some(first_list) = object_lists.first() else {
        return Err(ResolutionError::SetOperationFailed {
            set_id: _set_id.to_string(),
            reason: "COMPLEMENT requires at least one operand".to_string(),
        });
    };

    let Some(second_list) = object_lists.get(1) else {
        return Err(ResolutionError::SetOperationFailed {
            set_id: _set_id.to_string(),
            reason: "COMPLEMENT requires exactly 2 operands".to_string(),
        });
    };
