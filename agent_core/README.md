...
Agent Core (`agent_core`) consumes parsed ESP definitions from the `compiler`, resolves all dependencies and references, executes platform-specific data collection, and validates system state against compliance requirements.
...
| `contract_kit` | Builds on agent_core with `agent_core_api` high-level interface |
...
```rust
use agent_core::execution::{ExecutionEngine, ExecutionError};
...
```rust
use agent_core::execution::comparisons::ComparisonExt;
